...
export default function DetailScreen({ route }) {
  ...
  const openLink = (url) => Linking.openURL(url).catch(() => Alert.alert('Erro', 'Não foi possível abrir o link.'));
  ...
  const openMaps = () => openLink(`https://www.google.com/maps/dir/?api=1&destination=${attraction.lat},${attraction.lng}`);
    ...
    <ScrollView style={styles.container}>
      ...
      <View style={styles.content}>
        ...
        >
          ...
          />
          <Text style={[styles.interestBtnText, isInterested && { color: '#fff' }]}>
            {isInterested ? 'Remover dos Interesses' : 'Adicionar aos Interesses'}
          </Text>
        </TouchableOpacity>
        ...
        <TouchableOpacity style={styles.row} onPress={openMaps}>
          ...
          <Text style={[styles.infoText, styles.link]}> {attraction.address}</Text>
        </TouchableOpacity>
        ...
        <TouchableOpacity style={styles.contactRow} onPress={openEmail}>
          ...
          <Text style={[styles.infoText, styles.link]}> {attraction.email}</Text>
        </TouchableOpacity>
        <TouchableOpacity style={styles.contactRow} onPress={openPhone}>
          <Ionicons name="call-outline" size={18} color="#0077B6" />
          <Text style={[styles.infoText, styles.link]}> {attraction.phone}</Text>
        </TouchableOpacity>
        {attraction.whatsapp && (
        ...
        {attraction.hasReservation && (
          <TouchableOpacity style={styles.reserveBtn} onPress={openWhatsApp || openPhone}>
